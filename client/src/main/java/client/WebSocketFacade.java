...
public class WebSocketFacade extends Endpoint {
    ...
    final WebSocketResponseHandler defaultObserver = new WebSocketResponseHandler() {
    ...
    public WebSocketFacade(String url, WebSocketResponseHandler messageObserver) throws DeploymentException, IOException, URISyntaxException {
        URI uri = new URI(url);
        URI socketURI = new URI("ws", uri.getUserInfo(), uri.getHost(), uri.getPort(), "/ws", null,
                null);
    ...
    private void error(ErrorMessage message) {
        responseHandler.notify(String.format("ERROR: %s", message.getMessage()));
    }
